...
````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(woman, main="Box Plot WeeklyWages Woman", col="gray", xlab = "WeeklyWages", ylab = "Values")
boxplot(man, main="Box Plot WeeklyWages Man", col="gray", xlab = "WeeklyWages", ylab = "Values")
````

## 4.2 Interpretación 

Podemosobservar que en ámbos casos la mayoría de datos para ámbos sexos se localizan en el mismo intervalo, si podemos observar que para hombres existen más valores extremos que para las mujeres es decir que hay una mayor cantidad de  horas semanalaes que se encuentran poo encima del límite superior y más valores inferiores al límite inferior en hombres.
