...
`````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(formula = income  ~ gender, data = adult)
````

`````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(formula = income  ~ race, data = adult)
````

`````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(formula = income  ~ workclass, data = adult)
````

`````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(formula = income  ~ marital_status, data = adult)
````

`````{r echo=TRUE, message=FALSE, warning=FALSE}
boxplot(formula = income  ~ occupation, data = adult)
````
