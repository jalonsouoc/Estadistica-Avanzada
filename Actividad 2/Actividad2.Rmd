...
Aplicamos los cálculos relativos al contraste de dosmuestras emparejadas sobre la media
...
Podemos ver que el valor observado es de -20.801, que el valro critico por la izquierda es de -1.96, el valor crítico por la derecha es de 1.96 y el p valor es de 1.


## 3.4 Conclusión

Dado que el pvalor devuelto por la función es superior al alfa definido de 0.05 podemos definir que la hipótesis nula es cierta por tanto podemos aceptar que no hay diferencias entre IniCost y UltCost con un 95% de confinza.
...
Mostramos ahora los diagrama de cajas para los hombres y las mujeres de la variable WeeklyWages.
...
Podemos observar que en ámbos casos la mayoría de datos para ámbos sexos se localizan en el mismo intervalo, si podemos observar que para hombres existen más valores extremos que para las mujeres es decir que hay una mayor cantidad de salarios semanalaes que se encuentran por encima del límite superior y más valores inferiores al límite inferior en hombres que en mujeres.
...
Para este caso se aplicará un contraste de hipótesis sobre dos muestras, una para cada sexo, aplicado sobre la media del salario semanal y unilateralmente por la derecha dado que se pretende comprobar si el salario semalan de los hombres es mayor al de las mujeres.
...
Dado que el pvalor  es inferior al alfa definido de 0.05 podemos asumir que las varianzas son diferente por lo tanto aplicaremos el estadístico del contraste de dosmuestras sobre la media con varianzas desconocidas diferentes.
...
Observamos de nuevo que el pvalor devuelto es inferior al alfa definido por lo que comprobamso que se rechaza la hipótesis nula.
...
Para este caso se aplicará un contraste de hipótesis sobre dos muestras, una paracada sexo, aplicado sobre la media salarial semanal y unilateralmente por la derecha dado que s epretende comprobar si el salario promedio de los hombres es al menos 50 euros mayor que el de las mujeres.

## 5.3 Cálculos

\\ Para este caso el test de igualdad de varianzas nos sirve el del ejercicio anterior
...
Mostramos un diagrama de barras con las categorías de la variabl ePartTimeFullTime según el género.
...
De los gráficos obtenidos en el apartado anterior podemos observar que el porcentaje de mujeres que tienen jornada a tiempo parcial es mayor al porcentaje de los hombres que tienen dicha jornada. También podemos observar que la muestra posee un mayor número de registros para hombres que para mujeres. Podemos observar también que del total de registros hay un mayor número de mujeres que trabajan a tiempo parcial que de hombres.

## 6.3 Hipótesis nula y alternativa
Definimos la hipótesis nula y alternativa:

* **Hipótesis nula:** la proporción de mujeres que trabajan a tiempo completo es igual a la proporción de hombres que trabajan a tiempo completo
                                                        $$H_0: p_1 = p_2$$
* **Hipótesis alternativa:** la proporción de mujeres que trabajan a tiempo completo es mayor a la proporción de mujeres que trabajan a tiempo completo
                                                        $$H_1: p_1>p_2$$

## 6.4 TIpo de test

Para este caso se va a utilizar un contraste de hipótesis sobre dos muestras, una para hombre sy otra para mujeres, aplicado sobre la proporción del tipo de jornada laboral y unilateralmente por la derecha dado que se pretende comprobar que la proporción de mujeres sea mayor a la de hombres.
...
COn los resultados de los cálculos podemos rechazar la hipótesis nula debido a que el p-valor es inferior a nuestro alfa de 0.05, por lo que podemos concluir que la proporción de mujeres que trabajana tiempo parcial es superior a la proporción de hombres que trabajana a tiempo parcial con un nivel de confianza del 95%.

Podemos ver que el valor observado obtenido es 57.3423 y el valor crítico es de 1.64485.
