...
Para la pregunta definida, el contraste a aplicar es un contraste de dos muestras ya que por un lado tenemos la muestra de la variable IniCost y por el otro la muestra de la variable UltCost. El contraste se aplica sobre la varianza y es un contraste bilateral.
...
pvalor <- min(pf(fobs, df1=n1-1, df2=n2-2, lower.tail=FALSE), pf(fobs, df1=n1-1, df2=n2-2)) * 2
...
t.test(claim$IniCost,claim$UltCost)
````
...
Inicialmente realizamos el test de ...
...
fcritU <- qf(1 - alfa/2, df1=n1-2, df2=n2-2)
...
t <- dfMean/sqrt((s1^2/n1 + s2^2/n2))
pvalor <- pnorm(t, lower.tail = FALSE)
zcrit <- qnorm(alfa)
c(pvalor, zcrit)
